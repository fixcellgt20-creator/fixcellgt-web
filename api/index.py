#!/usr/bin/env python3
"""
API REST para FixCellGt - Desplegable en Vercel
...
def obtener_estadisticas():
    ...
    try:
        ...
        cursor.execute("""
            SELECT
                COUNT(DISTINCT id_modelo) as total_modelos,
                COUNT(DISTINCT ma.id_marca) as total_marcas,
                COUNT(DISTINCT tr.id_repuesto) as total_repuestos,
                ROUND(AVG(precio_reparacion)::numeric, 2) as precio_promedio,
                ROUND(MAX(precio_reparacion)::numeric, 2) as precio_maximo,
                ROUND(MIN(precio_reparacion)::numeric, 2) as precio_minimo,
                ROUND(SUM(margen_reparacion)::numeric, 2) as margen_total
            FROM inventario inv
            JOIN marcas ma ON inv.id_marca = ma.id_marca
            JOIN tipo_repuestos tr ON inv.id_repuesto = tr.id_repuesto
        """)
